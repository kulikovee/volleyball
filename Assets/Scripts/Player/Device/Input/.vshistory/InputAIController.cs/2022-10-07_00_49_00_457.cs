...
public class InputAIController : MonoBehaviour
{
    ...
    private readonlyfloat aiTargetUpdateTimeout = 0.15f;
    ...
    void Update()
    {
        ...
        float randomDeltaX = Random.RandomRange(0.2f, 1f) + extraTargetX;
        float randomDeltaZ = Random.RandomRange(0, transform.position.z) + extraTargetZ;
        ...
        var distance = Mathf.Clamp01(Vector3.Distance(transform.position, target.position) / 5);
