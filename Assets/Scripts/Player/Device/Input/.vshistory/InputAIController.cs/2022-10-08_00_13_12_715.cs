...
public class InputAIController : MonoBehaviour
{
    ...
    private void UpdateAxis()
    {
        ...
        float distance = Mathf.Clamp01(Vector3.Distance(transform.position, target.position) / 5f);
        ...
        aiControls += new Vector3((float)axisX, (float)axisY, 0);

        axis.SetX(Mathf.Clamp01((float)aiControls.x));
        axis.SetY(Mathf.Clamp01((float)aiControls.y));

        Debug.Log(axisX + " : " + Mathf.Clamp01((float)aiControls.x));
    }
}
