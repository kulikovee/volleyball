...
public class StartupMenuController : MonoBehaviour
{
    ...
    private int playerPreviousDevice0 = PlayerController.NO_DEVICE;
    private int playerPreviousDevice1 = PlayerController.NO_DEVICE;
    private int playerPreviousDevice2 = PlayerController.NO_DEVICE;
    private int playerPreviousDevice3 = PlayerController.NO_DEVICE;
    ...
    public void stopGame()
    {
        nextRound();
        ball.stopBall();
        players.ForEach(player => player.restartPlayer());
    ...
    {
        return player0.isDeviceEquals(deviceId) || player1.isDeviceEquals(deviceId) || player2.isDeviceEquals(deviceId) || player3.isDeviceEquals(deviceId);
    }
    ...
    {
        var deviceId = PlayerController.NO_DEVICE;

        KEYBOARD_WASD_KEYS.ForEach((keyCode) => {
            if (Input.GetKeyDown(keyCode))
            {
                deviceId = PlayerController.KEYBOARD_WASD;
            }
        });

        if (Input.GetKeyDown(KeyCode.Escape) && isDeviceSelected(PlayerController.KEYBOARD_WASD))
        {
            pause.updatePauseTimeout();
            timerFinished();
        }

        if (deviceId == PlayerController.NO_DEVICE)
        {
            KEYBOARD_ARROWS_KEYS.ForEach((keyCode) => {
                if (Input.GetKeyDown(keyCode))
                {
                    deviceId = PlayerController.KEYBOARD_ARROWS;
                }
            });
        }

        for (int gamePadId = 0; gamePadId < Gamepad.all.Count && deviceId == PlayerController.NO_DEVICE; gamePadId++)
        {
            if (!isDeviceSelected(gamePadId))
            {
                var gamePad = Gamepad.all[gamePadId];
                if (
                    deviceId == PlayerController.NO_DEVICE && (
        ...
        if (deviceId != PlayerController.NO_DEVICE && !isDeviceSelected(deviceId) && !isAllDevicesSelected())
        {
            joinSound.Play();
            addSecondsToTimer();
    ...
    private void setPlayerDevice(int deviceId)
    {
        ...
        if (deviceId == playerPreviousDevice0)
        {
            player = player0;
        ...
        if (deviceId == playerPreviousDevice1)
        {
            player = player1;
        ...
        if (deviceId == playerPreviousDevice2)
        {
            player = player2;
        ...
        if (deviceId == playerPreviousDevice3)
        {
            player = player3;
        ...
        Debug.Log(deviceId + ":" + player0.hasDevice() + "," + player1.hasDevice() + "," + player2.hasDevice() + "," + player3.hasDevice());
