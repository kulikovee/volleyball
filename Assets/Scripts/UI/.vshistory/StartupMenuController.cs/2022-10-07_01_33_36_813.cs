...
public class StartupMenuController : MonoBehaviour
{
    ...
    void CheckUserSelection()
    {
        ...
        if (deviceId == DeviceController.NO_DEVICE && InputNumpadController.IsPressed())
        {
            deviceId = DeviceController.KEYBOARD_NUMPAD;
        }

        if (deviceId == DeviceController.NO_DEVICE && InputGamepadController.IsPressed())
        {
            InputGamepadController.GetPressedIds().ForEach(gamePadId => {
